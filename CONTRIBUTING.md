...
Mocks will be automatically regenerated by `scripts/test-unit` and the
`scripts/run-acceptance-specs-*` scripts using `go generate ./...`. The
generated mocks are excluded from source control.
